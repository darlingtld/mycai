...
public class ExcelFactory {
    ...
    public static void exportProducts(String fileName, List<Product> productList, Map<Type, List<Category>> typeMap) {
        ...
        for (Type type : typeMap.keySet()) {
            ...
            headerList.add(new String[]{"名字", "描述", "大类", "小类", "采购价", "销售价", "单位"});
        }
        ...
        for (Type type : typeMap.keySet()) {
            ...
            for (Category category : categoryList) {
                ...
                for (Product product : subProductList) {
                    content.add(new String[]{product.getName(), product.getDescription(), product.getType().getType(), product.getCategory().getCategory(), product.getPrice().toString(), product.getPrice().toString(), product.getUnit()});
                }
            }
